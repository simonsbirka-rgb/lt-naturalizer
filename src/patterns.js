...
const patterns = [
    ...
    detect(text) {
      ...
      results.push(
        ...findMatches(
          text,
          /panagrinėkime/gi,
          "Replace with: pažiūrėk",
          "high",
        ),
      );
      results.push(
        ...findMatches(text, /tyrinėti/gi, "Replace with: pažvelgti", "high"),
      );
      results.push(
        ...findMatches(text, /atskleisti/gi, "Replace with: parodyti", "high"),
      );
      ...
      results.push(
        ...findMatches(
          text,
          /verta atkreipti dėmesį/gi,
          "Remove — just state the fact",
          "high",
        ),
      );
      results.push(
        ...findMatches(text, /neabejotinai/gi, "Replace with: tikrai", "high"),
      );
      ...
      results.push(
        ...findMatches(
          text,
          /nuolat besikeičiančiame kraštovaizdyje/gi,
          "Replace with: dabar",
          "high",
        ),
      );
      results.push(
        ...findMatches(
          text,
          /šiuolaikiniame skaitmeniniame amžiuje/gi,
          "Replace with: dabar",
          "high",
        ),
      );
      ...
      results.push(
        ...findMatches(
          text,
          /išlaisvinti potencialą/gi,
          "Replace with: padėti geriau",
          "high",
        ),
      );
      results.push(
        ...findMatches(text, /įgalinti/gi, "Replace with: padėti", "high"),
      );
...

  {
    ...
    detect(text) {
      ...
      results.push(
        ...findMatches(
          text,
          /Mes turime suprasti, kad/gi,
          'Remove "Mes" — use: Turime suprasti, kad',
          "high",
        ),
      );
      results.push(
...

  {
    ...
    detect(text) {
      ...
      results.push(
        ...findMatches(
          text,
          /priėmėme sprendimą/gi,
          "Replace with: mes nusprendėme",
          "high",
        ),
      );
      results.push(
        ...findMatches(
          text,
          /vykdyti tobulinimą/gi,
          "Replace with: tobulinti",
          "high",
        ),
      );
...

  {
    ...
    detect(text) {
      ...
      // For testing string "Įmonės plėtros strategijos įgyvendinimo plano optimizavimas yra sudėtingas procesas."
      // The genitives are: Įmonės, plėtros, strategijos, įgyvendinimo, plano
      const genitiveChain =
        /(?:\S+(?:o|io|ų|os|ės|ių|ies|aus|iaus)\s+){2,}\S+(?:o|io|ų|os|ės|ių|ies|aus|iaus)\b/gi;
      return findMatches(
        text,
        genitiveChain,
        "Rewrite using verbs instead of stacked genitive nouns (kilmininkų virtinė)",
        "high",
      );
    },
  },
        ...
        return [
          {
            ...
            column: 1,
            suggestion:
              "Add natural Lithuanian particles (juk, gi, turbūt, etc.) to make text sound more human",
            confidence: "medium",
          },
        ];
      }
...

  {
    ...
    category: "style",
    description:
      "Lists where each item starts with a bolded header followed by a colon.",
    weight: 3,
    detect(text) {
...

  {
    ...
    category: "style",
    description:
      "Capitalizing Every Main Word In Headings. AI chatbots default to this.",
    weight: 1,
    detect(text) {
...

  {
    ...
    category: "style",
    description:
      "Decorating headings or bullet points with emojis in professional/technical text.",
    weight: 2,
    detect(text) {
...

  {
    ...
    category: "language",
    description: "Imperfective verbs used where perfective prefix required.",
    weight: 3,
    detect(text) {
      const cues = [
        "vis\u0105",
...

  {
    ...
    category: "language",
    description: 'Using "vienas" as indefinite article like English "a/an".',
    weight: 3,
    detect(text) {
      return findMatches(
        text,
        /\bvienas\s+\w+(?:\s+(?:yra|buvo|bus|turi|tur\u0117jo))?\b/gi,
        'Pa\u0161alink "vienas" nebent rei\u0161kia skai\u010di\u0173 "one". LT neturi artikli\u0173.',
        "medium",
      );
    },
  },
    ...
    detect(text) {
      ...
      for (const s of sents) {
        ...
        if (
          ...
          svo++;
        if (
          /^(vakar|ryte|vakare|namie|mieste|\u0161iandien|rytoj|anksti)\b/i.test(
            s,
          )
        )
          fronted++;
...

  {
    ...
    detect(text) {
      ...
      for (let i = 0; i < paras.length; i++) {
        ...
        const informal =
          /\b(?:gi|juk|d\u0117lto|bene|turb\u016bt|gal|na)\b/.test(p);
        if (formal && informal) {
          results.push({
